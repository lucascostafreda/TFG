function [accuracyFitness, RB_usedAverage]  = runFLEnviroment_RUCHE_noQ(PI, iteration, averagenumber, r,...
    ruche,verbose, miniBatchSize, executionEnviroment, AccDevMat, Shuffle, fullpath_tempDir, FragSDS, percentages, iPI) 
    ...
    if AccDevMat
        ...
        if exist(fullfile(fullpath_tempDir, filename_dev), 'file')
            disp(['File saved: ', fullfile(fullpath_tempDir, filename_dev)]);
        else
            disp(['Failed to save file: ', fullfile(fullpath_tempDir, filename_dev)]);
        end
    end
