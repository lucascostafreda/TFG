function [deviationFitness, accuracyFitness, RB_usedAverage, elapsedTime] = EvaluatePopulation_joint_simetric(decodedPIs, r, available_RBs, iteration,...
...
r=[r,r];
available_RBs=available_RBs*2;

for index = 1:size(decodedPIs, 1)
    previousFitness = 0;
    previousAccuracy = 0;
    updated_PI_DEMI = decodedPIs(index,:);
    %%%%%%%%%% AQUI %%%%%%%%%%%%
    updated_PI = [updated_PI_DEMI,updated_PI_DEMI];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % if sum(updated_PI.* r) > available_RBs
    %     updated_PI = RepairOffspring(updated_PI, r, available_RBs);
    % end
        ...
        for iGen = iGeneration:-1:1
            ...
            if iGen == iGeneration && index > 1
              for iChrom = index-1:-1:1
                 if isequal(decodedPIs(iChrom, :), updated_PI_DEMI) %this iteration
            ...
            for iChrom = size(prevPopulation, 1):-1:1
                if isequal(prevPopulation(iChrom, :), updated_PI_DEMI)
                    if chromosomeRepetitions(piKey) >= 3
                        found = false;
                        chromosomeRepetitions(piKey) = 1; % Reset repetition counter
    ...
    fprintf('------------- Duplicated PI in Gen: %d , Excessed PI in Gen: %d ------------- \n\n', dupPI, excesPI);    
